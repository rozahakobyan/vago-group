...
class MinimalPriceController {
    static async add (req, res, next){
        try{
            const {eur, usd, rub, amd, min, active} = req.body;

            if(!eur || !usd || !rub || !amd || !min){
                throw HttpError(404, {
                    errors: {
                        exists: "Not found"
                    }
            ...
            const price = await MinimalPrice.create({eur, usd, rub, amd, min, active})
    ...
    static async update (req, res, next){
        try{
            ...
            await price.update({eur, usd, rub, amd, min, active})
